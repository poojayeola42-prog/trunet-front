...
// Returns the last valid date of the current month (e.g. 31 in August, 30 in September)
// instead of hardcoding a fixed day, so it works correctly for every month automatically.
const getLastDayOfMonth = () => {
  const now = new Date();
  const lastDay = new Date(now.getFullYear(), now.getMonth() + 1, 0);
  return lastDay.toISOString().split('T')[0];
